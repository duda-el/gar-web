...
const Projects = () => {
  ...
  return (
    <section className="relative py-24 px-6 overflow-hidden bg-[#121212]" id="projects">
            ...
            {filteredProjects.map((project) => (
              ...
              >
                ...
                <div className="mt-auto px-2 relative">
                  ...
                  <h3 className="text-xl md:text-2xl font-bold leading-tight tracking-tight text-white group-hover:text-[#f19035] transition-colors duration-200 font-georgian mb-3">{project.title}</h3>
                  <p className="text-sm text-zinc-300 font-georgian leading-relaxed line-clamp-4 italic">{project.description}</p>
                </div>
              </motion.div>
            ))}
          </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className="relative w-full max-w-4xl max-h-[90vh] bg-[#1c1c1c] border border-white/10 overflow-hidden z-[151] flex flex-col" style={{ borderRadius: "24px" }}>
              ...
              <div className="flex-1 overflow-y-auto p-8 md:p-12 bg-[#1c1c1c] custom-scrollbar">
                <div className="max-w-3xl mx-auto space-y-8">
                  <div>
                    <span className="text-[#f19035] text-[10px] tracking-[0.3em] font-bold uppercase mb-2 block font-georgian">{selectedProject.category}</span>
                    <h2 className="text-4xl font-bold text-white font-georgian tracking-tighter">{selectedProject.title}</h2>
                  </div>
                  <p className="text-zinc-400 font-georgian leading-relaxed italic border-l-2 border-[#f19035]/30 pl-4 text-lg">{selectedProject.description}</p>
                  <div className="space-y-4 pt-4">
                    <h4 className="text-white text-sm font-bold uppercase tracking-widest flex items-center gap-2 font-georgian">
                      <div className="w-4 h-[1px] bg-[#f19035]" /> ჩვენი სერვისები
                    </h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {selectedProject.services.map((service, idx) => (
                        <div key={idx} className="flex items-center gap-3 text-zinc-300 text-sm font-georgian">
                          <CheckCircle2 size={16} className="text-[#f19035] shrink-0" />
                          {service}
                        </div>
                      ))}
                    </div>
                  </div>
                  <span className="text-[#f19035] text-[10px] tracking-[0.3em] font-bold uppercase mb-2 block font-georgian">სტატუსი: {selectedProject.status}</span>
                </div>
              </div>
            </motion.div>
          </div>
